 function [ Satisfaction,Bandwidth_Enterprise,Bandwidth_Home,Throughput, Mean_Frame_Delay,Delay_Jitter, Frames_Drop ] = CPFCT( T,N,Rm,Load )
	%tic用来检测时间，不超出T
	tic
	%参数含义
	% T: 仿真时间
...
% 	end
	...
	for i=1:N
		Event_List(1,end+1) = 1;
		Event_List(2,end) = Time + poissrnd(10,1,1) * 10^-5/Load; %seconds (目的是为了对事件进行优先级的排序，按照时间先后对事件进行处理)
		Event_List(3,end) = 2;%优先级 （这是第二个需要考虑的优先级）
		Event_List(4,end) = i;%ONU_ID
	end
...
% 	V_transmit = 3*10^8; %metres/seconds
	...
	while Sim_Flag
            ...
            %第一行为为1则表示ONU产生数据包，为2则分配带宽，为3则表示ONU上传数据，为4则结束上行
			if Event == 1
			...
			elseif Event == 2
				[ Event_List, OLT_Buffer_Occupancy, Ending_Times_of_Transmissions, Total_Bytes_Sent, Total_Waiting_Time, Total_Frames_Sent, Buffer_Size, Bandwidth_Enterprise, Bandwidth_Home] = Event2 ( Time, Event_List, Bytes_ONU_Asks, N, OLT_Buffer_Occupancy, ONU_Buffer_Occupancy, RTT_2, BWmap_Size, Rm, Guard_Time, Report_msg_Size, Ending_Times_of_Transmissions, Total_Bytes_Sent, Total_Waiting_Time, Total_Frames_Sent, Buffer_Size, Bandwidth_Enterprise, Bandwidth_Home );
			elseif Event == 3
				[ Event_List, ONU_Buffer_Occupancy, Bytes_ONU_Asks, Total_Bytes_Sent, Total_Waiting_Time, Total_Frames_Sent, Buffer_Size, Frames_delays ] = Event3 ( Time, Event_List, ONU_Buffer_Occupancy, Bytes_ONU_Asks, Total_Bytes_Sent, Total_Waiting_Time, Total_Frames_Sent, Buffer_Size, Frames_delays);
			elseif Event == 4
			...
			Event_List(:,1)=[];
            %对事件的优先级进行排序
            %A'为A的转置,后面[2,3]表示先按第二行升序排列，若第二行相同，则按第三行排列，即先按时间排序，再按事件优先级排序
...
%         Frame_Size = randi([1000,1518],1);%生成[1000,1518]中一个随机大小的帧
%     else    
%         Frame_Size = randi([64,800],1);
%     end
    Frame_Size = randi([64,1518],1);%生成[64,1518]中一个随机大小的帧
    ...
    %产生新事件，每过一段时间产生数据包
    Event_List(1,end+1) = 1;
    Event_List(2,end) = Time + poissrnd(10,1,1) * 10^-5/Load; %seconds（因为总时间固定，load越大，时间间隔越小，可传输数据越多）
...
function [ Event_List, OLT_Buffer_Occupancy, Ending_Times_of_Transmissions, Total_Bytes_Sent, Total_Waiting_Time, Total_Frames_Sent, Buffer_Size, Bandwidth_Enterprise, Bandwidth_Home ] = Event2 ( Time, Event_List, Bytes_ONU_Asks, N, OLT_Buffer_Occupancy, ONU_Buffer_Occupancy, RTT_2, BWmap_Size, Rm, Guard_Time, Report_msg_Size, Ending_Times_of_Transmissions, Total_Bytes_Sent, Total_Waiting_Time, Total_Frames_Sent, Buffer_Size, Bandwidth_Enterprise, Bandwidth_Home )
    ...
    %上行容量为1G,这里把bit换成byte来计算
    Remaining_Bandwidth = (250*10^3-16*Guard_Time)/8;%Bytes
...
%                             Remaining_Bandwidth = Remaining_Bandwidth - Rm;
            ...
            %和Remaining_Bandwidth = Remaining_Bandwidth - (N - ONU_ID)*Rm相匹配，为了给后面的ONU保留最小带宽
            Remaining_Bandwidth = Remaining_Bandwidth + (N - ONU_ID)*Rm; %Bytes
        end
       
        %ONU开始传输信息的时间为:OLT的GRANT帧经过光纤传到ONU的时间，当有其他帧在传输时，自动往后延迟
        start_transmission_time = Time + RTT_2(1,ONU_ID);  
        if ONU_ID > 1
           if start_transmission_time < (Ending_Times_of_Transmissions(1,ONU_ID-1) + Guard_Time)
               start_transmission_time = (Ending_Times_of_Transmissions(1,ONU_ID-1) + Guard_Time);
           end
        end
        Ending_Times_of_Transmissions(1,ONU_ID) = start_transmission_time + Bandwidth_Distribution*8/(1*10^9);
    ...
    %每隔250us,OLT分配一次带宽
    Event_List(1,end+1) = 2;
    Event_List(2,end) = Time + 250*10^(-6);
    Event_List(3,end) = 1; %优先级
end

%Event 3:ONU上传数据包
function [ Event_List, ONU_Buffer_Occupancy, Bytes_ONU_Asks, Total_Bytes_Sent, Total_Waiting_Time, Total_Frames_Sent, Buffer_Size, Frames_delays ] = Event3 ( Time, Event_List, ONU_Buffer_Occupancy, Bytes_ONU_Asks, Total_Bytes_Sent, Total_Waiting_Time, Total_Frames_Sent, Buffer_Size, Frames_delays)
   T=Time;
   ONU_ID = Event_List(4,1);
